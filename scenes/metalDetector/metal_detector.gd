...
@onready var effectsBar: HSlider = $UI/OptionsUI/HScrollBar2
@onready var coinSound: AudioStreamPlayer2D = $AudioStreamPlayer2D
...
func _process(delta: float) -> void:
	coinSound.volume_db = linear_to_db(effectsBar.value)
	if gameOver:
		return
	if goldLeft == 0:
		timer.start()
